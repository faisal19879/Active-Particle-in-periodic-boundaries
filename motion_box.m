function [x,y,theta,v]=motion_box(x,y,DT,dt,A,theta,M,N,DR,V,Lx,Ly,R,VideoName)
fig1= figure();
...
for i = 1:N
  ...
  [x(i+1,:),y(i+1,:),theta(i+1,:)]=collision_agnese(x(i+1,:),y(i+1,:),M,R,theta(i+1,:));
  [x(i+1,:),y(i+1,:),theta(i+1,:)]=boundary_box(M,Lx,Ly,x(i+1,:),y(i+1,:),theta(i+1,:),R);
